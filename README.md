...
> Control the **Unreal Engine 5.8** editor from any **MCP client / AI agent** — two universal tools (`execute_python`, `execute_console_command`) layered on Epic's built-in **Model Context Protocol** server.
...
**Unreal MCP Toolkit is that work — done once, and universally.** Instead of dozens of narrow tools, it adds two that cover everything:
